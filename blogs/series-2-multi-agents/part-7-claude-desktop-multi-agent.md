...
This series covered one multi-agent pattern: the **Selector** — a router that delegates each turn to one specialist.

The next series in this repository will cover three more patterns:

**Pipe** — agents chain sequentially. The output of the job description agent becomes the input of a formatting agent, which feeds a translation agent. Each agent transforms rather than answers.

**Group Chat** — multiple agents run in parallel on the same query. A moderator synthesizes their responses. Useful for decisions that benefit from multiple perspectives — like evaluating a candidate from both a hiring manager and an HR compliance perspective simultaneously.

**Evaluator-Optimizer** — a critic agent scores a drafter's output and sends it back for revision until a quality threshold is met. The right pattern for generation tasks where first drafts are never good enough.

Each pattern will use the same HR domain, the same MCP servers, and the same clean architecture. The tools you built in this series — `WriteJobDescription`, `RunFullComplianceCheck`, `SaveJobAnnouncement` — all carry forward.
