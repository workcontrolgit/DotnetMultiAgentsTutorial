...
You now have the framework primitives: `IChatClient`, `UseFunctionInvocation`, MCP tool discovery, and the `SpecialistAgent` wrapper. The next post applies clean architecture principles to organise these primitives into a system that is testable, maintainable, and swap-friendly at every layer.
